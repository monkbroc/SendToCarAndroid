...
public class OnStarUploader extends BaseUploader {
	...
	private String preparePostDataOnStar() throws BackgroundTaskAbort {
		try {
			...
			// full address data
			final String[] codesMQ = { "s_lat", "s_long", "s_street", "s_city", "s_state_province", "s_postalcode", "s_country",
					"s_locale", "s_nounce", "s_entity_id", "SignatureMethod", "Signature" };
			final String[] valuesMQ = { address.latitude, address.longitude, streetaddress, address.city, address.province,
					address.postalCode, address.country, "en_US", timestamp, " " + timestamp, "https://mapquest.com.onstar.com",
					"RSA-SHA256", "" };
